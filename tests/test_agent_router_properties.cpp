...
TEST_F(AgentRouterPropertyTest, RoutingStrategyStringRoundTripSupportsConsistentHash) {
    EXPECT_EQ(to_string(RoutingStrategy::CONSISTENT_HASH), "consistent_hash");
    EXPECT_EQ(routing_strategy_from_string("consistent_hash"), RoutingStrategy::CONSISTENT_HASH);
}

TEST_F(AgentRouterPropertyTest, ConsistentHashUsesStableRoutingKeyAffinity) {
    router_->setStrategy(RoutingStrategy::CONSISTENT_HASH);

    for (int i = 0; i < 4; ++i) {
        router_->addAgent(createAgent("eval-" + std::to_string(i), {"evaluator"}));
    }

    std::string first_id;
    for (int i = 0; i < 10; ++i) {
        auto selected = router_->selectAgent("risk check", {"evaluator"}, "ctx-42");
        ASSERT_TRUE(selected.has_value());
        if (first_id.empty()) {
            first_id = selected->id;
        }
        EXPECT_EQ(selected->id, first_id);
    }
}

TEST_F(AgentRouterPropertyTest, ConsistentHashDistributesAcrossMultipleAgents) {
    router_->setStrategy(RoutingStrategy::CONSISTENT_HASH);
    ...
    for (int i = 0; i < 200; ++i) {
        auto selected = router_->selectAgent("risk check", {"evaluator"}, "ctx-" + std::to_string(i));
        ASSERT_TRUE(selected.has_value());
        selected_ids.insert(selected->id);
    }

    EXPECT_GE(selected_ids.size(), 3u);
}

TEST_F(AgentRouterPropertyTest, ConsistentHashRespectsRequiredSkills) {
    router_->setStrategy(RoutingStrategy::CONSISTENT_HASH);

    router_->addAgent(createAgent("eval-1", {"evaluator", "general"}));
    router_->addAgent(createAgent("eval-2", {"evaluator"}));
    router_->addAgent(createAgent("code-1", {"coding"}));

    for (int i = 0; i < 50; ++i) {
        auto selected = router_->selectAgent("risk check", {"evaluator"}, "ctx-" + std::to_string(i));
        ASSERT_TRUE(selected.has_value());
        EXPECT_TRUE(selected->hasSkill("evaluator"));
        EXPECT_FALSE(selected->hasSkill("coding"));
    }
}

TEST_F(AgentRouterPropertyTest, ConsistentHashReturnsOnlyMatchingAgentWhenSingleCandidate) {
    router_->setStrategy(RoutingStrategy::CONSISTENT_HASH);

    router_->addAgent(createAgent("eval-1", {"evaluator"}));
    router_->addAgent(createAgent("code-1", {"coding"}));

    auto selected = router_->selectAgent("risk check", {"evaluator"}, "ctx-single");

    ASSERT_TRUE(selected.has_value());
    EXPECT_EQ(selected->id, "eval-1");
}

TEST_F(AgentRouterPropertyTest, ConsistentHashOnlyRemapsKeysFromRemovedAgent) {
    router_->setStrategy(RoutingStrategy::CONSISTENT_HASH);

    std::vector<std::string> agent_ids = {"eval-a", "eval-b", "eval-c", "eval-d"};
    for (const auto& id : agent_ids) {
        router_->addAgent(createAgent(id, {"evaluator"}));
    }

    std::unordered_map<std::string, std::string> before;
    for (int i = 0; i < 200; ++i) {
        const std::string key = "ctx-" + std::to_string(i);
        auto selected = router_->selectAgent("risk check", {"evaluator"}, key);
        ASSERT_TRUE(selected.has_value());
        before.emplace(key, selected->id);
    }

    const std::string removed_agent = "eval-b";
    router_->markAgentUnhealthy(removed_agent);

    for (int i = 0; i < 200; ++i) {
        const std::string key = "ctx-" + std::to_string(i);
        auto selected = router_->selectAgent("risk check", {"evaluator"}, key);
        ASSERT_TRUE(selected.has_value());
        if (before[key] == removed_agent) {
            EXPECT_NE(selected->id, removed_agent);
        } else {
            EXPECT_EQ(selected->id, before[key]);
        }
    }
}
