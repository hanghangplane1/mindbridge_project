...
static void set_program_autoload(struct bpf_program *prog, bool autoload)
{
	if (prog)
		bpf_program__set_autoload(prog, autoload);
}

static void configure_program_autoload(struct process_new_bpf *skel)
{
	/* Default to the small, stable surface: process lifecycle and file-open.
	 * Optional probes are loaded only when requested so disabled capabilities
	 * are not still sent through verifier on older kernels.
	 */
	set_program_autoload(skel->progs.trace_unlinkat, env.trace_fs);
	set_program_autoload(skel->progs.trace_unlink, env.trace_fs);
	set_program_autoload(skel->progs.trace_renameat2, env.trace_fs);
	set_program_autoload(skel->progs.trace_renameat, env.trace_fs);
	set_program_autoload(skel->progs.trace_rename, env.trace_fs);
	set_program_autoload(skel->progs.trace_mkdirat, env.trace_fs);
	set_program_autoload(skel->progs.trace_mkdir, env.trace_fs);
	set_program_autoload(skel->progs.trace_ftruncate, env.trace_fs);
	set_program_autoload(skel->progs.trace_chdir, env.trace_fs);
	set_program_autoload(skel->progs.trace_write_enter, env.trace_fs);
	set_program_autoload(skel->progs.trace_write_exit, env.trace_fs);
	set_program_autoload(skel->progs.trace_pwrite64_enter, env.trace_fs);
	set_program_autoload(skel->progs.trace_pwrite64_exit, env.trace_fs);
	set_program_autoload(skel->progs.trace_writev_enter, env.trace_fs);
	set_program_autoload(skel->progs.trace_writev_exit, env.trace_fs);

	set_program_autoload(skel->progs.trace_bind, env.trace_net);
	set_program_autoload(skel->progs.trace_listen, env.trace_net);
	set_program_autoload(skel->progs.trace_connect, env.trace_net);

	set_program_autoload(skel->progs.trace_setpgid, env.trace_signals);
	set_program_autoload(skel->progs.trace_setsid, env.trace_signals);
	set_program_autoload(skel->progs.trace_kill, env.trace_signals);
	set_program_autoload(skel->progs.trace_fork, env.trace_signals);

	set_program_autoload(skel->progs.trace_mmap, env.trace_mem);
	set_program_autoload(skel->progs.trace_cow_fault, env.trace_cow);
}
...
int main(int argc, char **argv)
{
	...
	configure_program_autoload(skel);
	...
	/* Also check MINDBRIDGE_EBPF_PID environment variable at runtime */
	if (env.trace_resources && g_resource_target_pid == 0) {
		const char *env_pid = getenv("MINDBRIDGE_EBPF_PID");
		if (env_pid && env_pid[0]) {
			pid_t pid = (pid_t)strtol(env_pid, NULL, 10);
			if (pid > 0) {
				g_resource_target_pid = pid;
				if (env.verbose)
					fprintf(stderr, "Set resource target PID from env: %d\n", pid);
			}
		}
	}
	...
	while (!exiting) {
		/* Check MINDBRIDGE_EBPF_PID environment variable at runtime */
		if (env.trace_resources && g_resource_target_pid == 0) {
			const char *env_pid = getenv("MINDBRIDGE_EBPF_PID");
			if (env_pid && env_pid[0]) {
				pid_t pid = (pid_t)strtol(env_pid, NULL, 10);
			...
			/* Also try to read from PID file */
			if (g_resource_target_pid == 0) {
				FILE *pid_file = fopen("/tmp/mindbridge_counselor.pid", "r");
				if (pid_file) {
					pid_t pid;
					if (fscanf(pid_file, "%d", &pid) == 1 && pid > 0) {
						g_resource_target_pid = pid;
						if (env.verbose)
							fprintf(stderr, "Set resource target PID from file: %d\n", pid);
					}
					fclose(pid_file);
				}
			}
