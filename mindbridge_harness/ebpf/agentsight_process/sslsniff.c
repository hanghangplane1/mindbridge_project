...
#ifndef LIBBPF_OPTS
#define LIBBPF_OPTS(TYPE, NAME, ...) DECLARE_LIBBPF_OPTS(TYPE, NAME, __VA_ARGS__)
#endif
...
static int resolve_symbol_offset(const char *binary_path, const char *sym_name,
								 size_t *offset)
{
	int fd = -1;
	Elf *elf = NULL;
	...
	if (!binary_path || !sym_name || !offset)
		return -1;
	...
	while ((scn = elf_nextscn(elf, scn)) != NULL) {
		...
		while ((data = elf_getdata(scn, data)) != NULL) {
			const size_t count = shdr.sh_entsize ? data->d_size / shdr.sh_entsize : 0;
...
static struct bpf_link *attach_symbol(struct bpf_program *prog, pid_t pid,
									  const char *binary_path, const char *sym_name,
									  bool retprobe)
{
	size_t offset = 0;
	if (resolve_symbol_offset(binary_path, sym_name, &offset) != 0)
		return NULL;
	return bpf_program__attach_uprobe(prog, retprobe, pid, binary_path, offset);
}

#define __ATTACH_UPROBE(skel, binary_path, sym_name, prog_name, is_retprobe)   \
	do {                                                                       \
	  skel->links.prog_name = attach_symbol(skel->progs.prog_name, env.pid,    \
											binary_path, #sym_name, is_retprobe); \
	} while (false)
...
#define __ATTACH_UPROBE_OFFSET(skel, binary_path, offset, prog_name, is_retprobe) \
	do {                                                                          \
	  skel->links.prog_name = bpf_program__attach_uprobe(                        \
		  skel->progs.prog_name, is_retprobe, env.pid, binary_path, offset);      \
	} while (false)
...
int attach_openssl_container(struct sslsniff_bpf *skel, const char *lib) {
    ...
    for (int i = 0; i < 2; i++) {
        /* entry */
        link = attach_symbol(entry_progs[i], -1, lib, sym_names[i], false);
        ...
        /* return */
        link = attach_symbol(exit_progs[i], -1, lib, sym_names[i], true);
        if (link && !libbpf_get_error(link)) {
            add_dynamic_link(link);
            attached++;
        }
    ...
    for (int i = 0; i < 2; i++) {
        link = attach_symbol(ex_entry_progs[i], -1, lib, ex_sym_names[i], false);
        if (link && !libbpf_get_error(link)) {
            add_dynamic_link(link);
            attached++;
        }
        ...
        link = attach_symbol(ex_exit_progs[i], -1, lib, ex_sym_names[i], true);
        if (link && !libbpf_get_error(link)) {
            add_dynamic_link(link);
            attached++;
        }
    ...
    /* SSL_do_handshake */
    link = attach_symbol(skel->progs.probe_SSL_do_handshake_enter, -1, lib, "SSL_do_handshake", false);
    if (link && !libbpf_get_error(link)) { add_dynamic_link(link); attached++; }
    else { link = NULL; }

    link = attach_symbol(skel->progs.probe_SSL_do_handshake_exit, -1, lib, "SSL_do_handshake", true);
    if (link && !libbpf_get_error(link)) { add_dynamic_link(link); attached++; }
    else { link = NULL; }
...
int main(int argc, char **argv) {
	...
	if (env.extra_lib) {
		...
		struct bpf_link *test_link = attach_symbol(
			obj->progs.probe_SSL_rw_enter, env.pid, env.extra_lib, "SSL_write", false);
		if (test_link) {
