...
constexpr std::size_t kVirtualNodesPerAgent = 128;

std::uint64_t fnv1a_hash(const std::string& input) {
...
std::optional<AgentInfo> AgentRouter::selectAgent(
    ...
    if (skills_to_match.empty() &&
        (strategy_ == RoutingStrategy::SKILL_MATCH ||
         strategy_ == RoutingStrategy::CONSISTENT_HASH)) {
    ...
    return selectByStrategy(candidates, resolveRoutingKey(question, required_skills, routing_key));
}
...
AgentInfo AgentRouter::selectByStrategy(const std::vector<AgentInfo>& candidates,
    ...
    switch (strategy_) {
        ...
        case RoutingStrategy::CONSISTENT_HASH:
            return selectConsistentHash(candidates, routing_key);
...
AgentInfo AgentRouter::selectConsistentHash(const std::vector<AgentInfo>& candidates,
                                            const std::string& routing_key) {
    std::vector<std::pair<std::uint64_t, std::size_t>> ring;
    ring.reserve(candidates.size() * kVirtualNodesPerAgent);

    for (std::size_t candidate_index = 0; candidate_index < candidates.size(); ++candidate_index) {
        const auto& agent = candidates[candidate_index];
        for (std::size_t vnode = 0; vnode < kVirtualNodesPerAgent; ++vnode) {
            ring.emplace_back(
                fnv1a_hash(agent.id + "#" + std::to_string(vnode)),
                candidate_index);
        }
    }
    ...
    const std::uint64_t key_hash = fnv1a_hash(routing_key);
    const auto it = std::lower_bound(
        ring.begin(), ring.end(), key_hash,
        [](const auto& entry, std::uint64_t value) { return entry.first < value; });

    if (it == ring.end()) {
        return candidates[ring.front().second];
    }
    return candidates[it->second];
}

std::string AgentRouter::resolveRoutingKey(const std::string& question,
                                           const std::vector<std::string>& required_skills,
    ...
    if (!required_skills.empty()) {
        ...
        for (std::size_t i = 0; i < required_skills.size(); ++i) {
            if (i > 0) {
                out << ",";
            }
            out << required_skills[i];
        }
        return out.str();
    }
    return "default";
